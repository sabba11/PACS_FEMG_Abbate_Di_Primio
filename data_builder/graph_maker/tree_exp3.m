...
% ndim = 3;
% Output file name
% out_filename = '../data/txt_files/tree_exp3_3.txt';
% out_filename = '../data/txt_files/tree_exp3_4.txt';
out_filename = '../data/txt_files/tree_exp3_5.txt';
...
for j = 2:nbif
    ...
    for i = 1:2^(j-2)
        ...
        points(counter,:) = [points(counter-1-3*(i-1),1)+cos(theta)*length_edge/(3^(j-1)),
            points(counter-1-3*(i-1),2)+sin(theta)*length_edge/(3^(j-1))];
        points(counter+1,:) = [points(counter-1-3*(i-1),1)+cos(theta)*length_edge/(3^(j-1)),
            points(counter-1-3*(i-1),2)-sin(theta)*length_edge/(3^(j-1))];
